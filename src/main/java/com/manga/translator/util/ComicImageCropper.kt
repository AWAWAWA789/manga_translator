...
object ComicImageCropper {
    ...
    fun adjustRectForCrop(rect: Rect, config: ScreenCropConfig, originalWidth: Int, originalHeight: Int): Rect {
        val cropLeft = (originalWidth * config.leftRatio).toInt()
        val cropTop = (originalHeight * config.topRatio).toInt()
    ...
    fun scaleRect(rect: Rect, scaleX: Float, scaleY: Float): Rect {
        return Rect(
