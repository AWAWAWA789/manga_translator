...
class OcrProcessor(private val context: Context) {
    ...
    fun recognizeText(bitmap: Bitmap, verticalOnly: Boolean = false): List<OcrResult> {
        ...
        val enhancedHorizontalResults = recognizeWithRotation(enhancedBitmap, 0f, scaledBitmap.width, scaledBitmap.height)
        val enhancedVerticalResults = recognizeWithRotation(enhancedBitmap, 90f, scaledBitmap.width, scaledBitmap.height)
        val rawHorizontalResults = recognizeWithRotation(scaledBitmap, 0f, scaledBitmap.width, scaledBitmap.height)
        val rawVerticalResults = recognizeWithRotation(scaledBitmap, 90f, scaledBitmap.width, scaledBitmap.height)
        val binaryHorizontalResults = recognizeWithRotation(binaryBitmap, 0f, scaledBitmap.width, scaledBitmap.height)
        val binaryVerticalResults = recognizeWithRotation(binaryBitmap, 90f, scaledBitmap.width, scaledBitmap.height)

        val horizontalResults = deduplicateResults(deduplicateResults(enhancedHorizontalResults, rawHorizontalResults), binaryHorizontalResults)
        val verticalResults = deduplicateResults(deduplicateResults(enhancedVerticalResults, rawVerticalResults), binaryVerticalResults)
        Log.d(TAG, "OCR输入统计: raw=${rawHorizontalResults.size + rawVerticalResults.size}, enhanced=${enhancedHorizontalResults.size + enhancedVerticalResults.size}, binary=${binaryHorizontalResults.size + binaryVerticalResults.size}")
        Log.d(TAG, "横向识别: ${horizontalResults.size} 个，竖向识别: ${verticalResults.size} 个")

        val allResults = if (verticalOnly) {
            deduplicateResults(verticalResults, horizontalResults)
        } else {
            deduplicateResults(horizontalResults, verticalResults)
        }
        ...
        if (enhancedBitmap !== scaledBitmap) enhancedBitmap.recycle()
        if (binaryBitmap !== scaledBitmap) binaryBitmap.recycle()
    ...
    /**
     * 按照竖向阅读顺序排序OCR结果
     * 日本漫画竖向阅读顺序：从上到下读取每一列，从右到左依次读取每一列
     */
    fun sortByVerticalReadingOrder(results: List<OcrResult>): List<OcrResult> {
        if (results.size <= 1) return results
        
        // 按X坐标从右到左排序（X坐标越大越靠右）
        return results.sortedWith(compareByDescending<OcrResult> { 
    ...
    private fun isOverlapping(r1: Rect?, r2: Rect?, threshold: Int): Boolean {
