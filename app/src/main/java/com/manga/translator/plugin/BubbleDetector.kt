...
class BubbleDetector {
    ...
    fun findBubbleForText(bubbles: List<BubbleInfo>, textRect: Rect): BubbleInfo? {
        for (bubble in bubbles) {
            if (isRectInsideRect(textRect, bubble.rect)) {
                return bubble
            }
        }
        
        var bestBubble: BubbleInfo? = null
        var bestDistance = Double.MAX_VALUE
        
        for (bubble in bubbles) {
            val distance = calculateDistance(textRect, bubble.rect)
        ...
        val threshold = (bestBubble?.rect?.width() ?: 0) * 0.5
        return if (bestDistance < threshold) bestBubble else null
    }
    
    private fun isRectInsideRect(inner: Rect, outer: Rect): Boolean {
    ...
    private fun calculateDistance(rect1: Rect, rect2: Rect): Double {
