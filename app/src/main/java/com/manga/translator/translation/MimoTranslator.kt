...
class MimoTranslator(private val context: Context) {
    ...
    fun translateWithContext(currentText: String, contextTexts: List<String>): String {
        if (!isConfigured()) {
            return "请配置MiMo API"
        }
        
        return try {
            val prompt = buildContextPrompt(currentText, contextTexts)
            cleanOutput(callMimoApi(prompt))
        } catch (e: Exception) {
            Log.e(TAG, "翻译失败: ${e.message}")
            "翻译失败: ${e.message}"
...
$text"""
    ...
    private fun buildContextPrompt(currentText: String, contextTexts: List<String>): String {
        val contextStr = contextTexts.mapIndexed { index, text -> 
            "${index + 1}. $text"
        }.joinToString("\n")
        
        return """你是专业漫画汉化翻译。以下是同一漫画页面的日文对白，已按阅读顺序排列。
